...
const Footer = ({ theme, openContact, settings }) => {
    return (
        <footer id="contact-footer" className={`transition-colors duration-500 py-16 border-t ${theme === 'dark' ? 'bg-brand-card border-brand-secondary' : 'bg-white border-slate-200 shadow-[0_-1px_3px_rgba(0,0,0,0.05)]'
            }`}>
            <div className="container mx-auto px-4">
                ...
                <div className={`border-t pt-10 flex flex-col lg:flex-row justify-between items-center gap-6 text-[10px] font-black uppercase tracking-widest transition-colors ${theme === 'dark' ? 'border-brand-secondary text-brand-muted/60' : 'border-slate-100 text-slate-400'
                    ...
                    <p className="text-center lg:text-left">
                        ...
                        <span className={`ml-4 font-bold ${theme === 'dark' ? 'text-white/40' : 'text-slate-400'}`}>Version: {settings?.system_version || '2026_02_07_V8'}</span>
                    </p>
